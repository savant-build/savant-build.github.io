...
By default, the plugin doesn't clean up the `WEB-INF/classes` directory. You can place configuration files in that directory, and the plugin will leave them there. However, some projects prefer to store configuration files outside the web application directory and copy them into the web application during the build. To force the plugin to clean the `WEB-INF/classes` directory, you can set the `cleanClassesDirectory` field on the settings object. Here is an example of setting this:
...
If you enable cleaning of the `WEB-INF/classes` directory and want to store your web resources outside the web application, you can use the `copyResources` and `webResourceDirectory` fields on the settings class. These fields control the location of the web resources and if they are copied to the web application during the build. Here is an example of setting these:
...
In most cases, you need to copy the JAR file for your project into the `WEB-INF/lib` directory. To do this, the plugin needs to know where the JAR is located. The `jarOutputDirectory` specifies where the project JAR files are located. Here is an example of setting this:
...
Some projects use a different web application directory other than `web`. You can change the location of the web application in your project by setting the `webDirectory` field on the settings object. Here is an example of setting this:
...
The directory where the WAR file is created. This defaults to `build/wars`:

~~~~ groovy
webapp.settings.warOutputDirectory = Paths.get("build/wars")
~~~~
...
To clean out the dependency JAR files and configuration files from the web application in your project, call the `clean` method like this:
