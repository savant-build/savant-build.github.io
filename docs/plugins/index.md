...
Solution #1 quickly becomes messy and difficult to manage. What if you want to inject some new build logic between the `compile` build target and the `test` build target? This is challenging because there is a strong coupling between the **Java** and **JUnit** plugins that is challenging to modify.
...
Savant plugins are simple objects. A build file can invoke methods on the object to perform a set of tasks. This method makes it simple to write build files using Savant plugins.

Here is an example of how simple it is to compile C# code using Savant:

~~~~ groovy
java = loadPlugin("org.savantbuild.plugin:java:2.2.0")
