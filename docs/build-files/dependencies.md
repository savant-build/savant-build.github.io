...
This tells Savant to first check the local cache for dependencies. If they aren't found there, download them from either the Savant repository or the Maven repository (Maven Central) and cache them locally (that's what the `publish` section is for).
...
The local cache for Savant is stored at **~/.cache/savant** (or `$XDG_CACHE_HOME/savant` if `XDG_CACHE_HOME` is set)
...
* cache - Fetches and stores dependencies in the local cache
  * directory - (Optional) If this is blank, Savant will use the default location of **~/.cache/savant**
* url - Fetches artifacts from a Savant repository at an HTTP/HTTPS URL
